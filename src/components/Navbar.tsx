...
export default function Navbar({ onNavigate, activeSection, isDark, onToggleDark }: NavbarProps) {
  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        mobileMenuOpen
          ...
          : isScrolled
          ? isDark ? 'bg-slate-900 shadow-lg shadow-slate-950/30 border-b border-white/10 py-3.5' : 'bg-white shadow-lg shadow-slate-200/50 border-b border-slate-200 py-3.5'
    ...
    >
      ...
      {/* Mobile Drawer menu */}
      {mobileMenuOpen && (
        <div className={`lg:hidden fixed inset-x-0 bottom-0 top-[-1px] z-40 border-t shadow-2xl transition-all duration-300 ${
